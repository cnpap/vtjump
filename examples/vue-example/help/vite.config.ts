        ...
        if (template) {
          ...
          function traverseNode(node) {
            if (node.type === 1 && isHTMLTag(node.tag)) { // 1 表示 ELEMENT 类型节点，且是内置 HTML 标签
              ...
              s.appendLeft(start + node.tag.length + 1, ` data-start-line="${startLine}" data-end-line="${endLine}"`);
            }
