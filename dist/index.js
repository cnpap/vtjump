...
const N = (o = {}) => {
  ...
  return {
    name: "vite:vtjump",
    configureServer(e) {
      r = e, r.middlewares.use(async (n, t, s) => {
        if (n.url === "/__vtjump") {
          const f = [];
          n.on("data", (l) => f.push(l)), n.on("end", () => {
            const l = JSON.parse(Buffer.concat(f).toString()), { id: g, getInfo: p } = l, c = b.get(g);
            if (c)
              if (p)
                t.setHeader("Content-Type", "application/json"), t.end(JSON.stringify({ location: c }));
              else {
                const i = o.protocol || "vscode", d = `${c.file}:${c.startLine}`;
                if (o.clientSideOpen)
                  t.setHeader("Content-Type", "application/json"), t.end(JSON.stringify({ url: `${i}://file/${d}` }));
                else {
                  const a = `${i}://file/${d}`, v = process.platform === "win32" ? `start ${a}` : process.platform === "darwin" ? `open "${a}"` : `xdg-open "${a}"`;
                  y.exec(v, (u) => {
                    if (u) {
                      console.warn("Failed to open URL with system command:", u);
                      const x = `${i} "${d}"`;
                      y.exec(x, (m) => {
                        m ? (console.error("Failed to open with protocol command:", m), t.statusCode = 500, t.end(JSON.stringify({ error: "Failed to open file" }))) : t.end(JSON.stringify({ success: !0 }));
                      });
                    } else
                      t.end(JSON.stringify({ success: !0 }));
                  });
                }
              }
            else
              t.statusCode = 404, t.end(JSON.stringify({ error: "Location not found" }));
          });
          return;
        }
        s();
    ...
    transform(e, n) {
      ...
      const { descriptor: t } = w(e);
      if (!t.template) return;
      const s = new h(e), f = t.template.content;
      t.template.loc.start.line;
      const l = t.template.loc.start.offset, g = /<(\w+)([^>]*)>/g;
      let p;
      for (; (p = g.exec(f)) !== null; ) {
        const [c, i, d] = p;
        if (!L(i) || d.includes("data-vtjump")) continue;
        const a = l + p.index, u = e.slice(0, a).split(`
`).length, x = M(n, u), m = a + i.length + 1;
        s.appendLeft(m, ` data-vtjump="${x}"`);
      }
      return {
        code: s.toString(),
        map: s.generateMap()
    ...
    transformIndexHtml(e) {
      const n = new h(e), t = e.match(/<\/body>/i);
      return t && n.appendLeft(
        t.index,
        `${C()}${$()}</body>`
      ), {
        html: n.toString(),
